...
(() => {
    ...
    // 轮盘链接
    const Vroll = `https://keylol.com/plugin.php?id=steamcn_lottery:view&lottery_id=${LootID}`;
    // 正则表达式
    const Vregex = new RegExp(`/plugin\.php\?id=steamcn_lottery:view&lottery_id=${LO}&hash=(.+)&roll/`);
    loadCFG();
    addBtns();
    if (VShow) {
        switchPanel();
    ...
    function switchPanel() {
        ...
        if (panel1.style.display == 'none') {
            btnSwitch.textContent = 'Auto Sub 3 - By Chr_ - V ' + version;
